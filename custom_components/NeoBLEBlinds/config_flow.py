"""Config flow for Neo Blinds integration."""
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_bluetooth(self, discovery_info: BluetoothServiceInfoBleak) -> FlowResult:
        ...
        if not discovery_info.name.startswith("NEO"):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            self._abort_if_unique_id_configured()
            device = NeoInstance(discovery_info.address)
            try:
                await device.update()
                ...
                return self.async_create_entry(title=local_name,data={CONF_NAME: local_name, CONF_ADDRESS: discovery_info.address})
